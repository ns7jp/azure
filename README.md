...
| 11 | [Azure基礎用語集(初心者向け)](docs/11-azure-glossary.md) | Azureと現場の基礎用語を、覚えたうえで自分の言葉で説明できる |

> [!TIP]
> 用語が分からなくなったら、順番どおりに進むより [Azure基礎用語集(初心者向け)](docs/11-azure-glossary.md) を辞書として引くほうが速いです。最初の20語だけを覚えれば、2番以降の設計書はほぼ読めます。
...
| IaC | `infra/main.bicep`(`scripts/deploy.ps1`等と連携、What-If前提) | `infra/advanced/main.bicep`(学習用の参考実装。CIで`az bicep build`まで検証済み、What-If以降は利用者が実施) |
