...
| シグナル・しきい値・集約粒度・評価頻度 | シグナル / しきいち / しゅうやくりゅうど / ひょうかひんど | アラートの「何を・いくらで・どの幅で・どの間隔で」の4点セット | 何を / いくらで / どの幅で / どの間隔で |
| アラートの重大度 | じゅうだいど | 0から4の数字で表すアラートの深刻さ | 0が最も重い。順位と同じで小さいほど上位 |
| アクショングループ | アクション グループ | アラート発生時の通知先と実行動作をまとめた連絡網 | 発火したあとの「誰に何を」 |
| アクティビティログ | アクティビティ ログ | 誰がAzureの構成をいつ変更したかの操作履歴 | 中身ではなく操作の履歴 |
| 診断設定 | しんだんせってい | リソースのログとメトリックの送り先を指定する配管 | 蛇口をどのバケツにつなぐか |
...
| 自動シャットダウン | じどうシャットダウン | 指定時刻にVMを自動で停止するスケジュール設定 | 消し忘れ防止のタイマー |
| ダッシュボードとブック | ダッシュボード / ブック | 常時見る計器盤と、調査用に読ませるレポート | 毎日見る盤と、読ませる資料 |
| ランブック | ランブック | 誰がやっても同じ結果になる運用手順書 | runとbook。走らせる本 |
| 一次切り分けとエスカレーション | いちじきりわけ / エスカレーション | 自分で切り分けられる範囲を調べ、超えたら引き継ぐこと | 止・見・守・戻・残 |
