...
> [!IMPORTANT]
> このセッションは **Azureの課金が発生する**。`-Apply` を付けた時点でリソースが作成され、フェーズ7で削除するまで課金が続く。開始前に予算アラートと削除手順を確認し、途中で中断する場合も必ずフェーズ7まで戻ること。

## 所要時間と費用の目安

| 項目 | 目安 |
|---|---|
| 所要時間 | 3〜5時間（ロードマップの見積り。初回は長めに見る） |
| 稼働させるリソース | `Standard_B1s` VM 1台、Public IP、OSディスク、Log Analytics（データほぼ無し） |
| 費用感 | 数時間の稼働なら小額に収まる構成だが、料金はリージョン・契約・時期で変わる。**実施直前に Azure Pricing Calculator で確認**し、実績は終了後に Cost Management で見る |

費用を増やす主な要因は「削除し忘れ」である。VMの停止だけではPublic IPやディスクの課金が残る（05「構築後の安全化」）。
...
フェーズ5 接続・OS試験 ──── NT-01, OS-01, OS-02, NT-03, (NT-04), NT-02
フェーズ6 再現性と失敗記録 ─ OP-01 + 意図的な不一致1件
フェーズ7 削除と残存確認 ── CL-01                ← ここで課金停止
フェーズ8 証跡の仕上げとコミット
```
...
# 編集: owner, sshPublicKey(.pubの1行), adminCidr(自分のIP/32), openHttp=false
git status --short   # dev.bicepparam が表示されない(.gitignore対象)ことを確認
```

`sshPublicKey` に `REPLACE_WITH_YOUR_PUBLIC_KEY` が残っている、`adminCidr` が `0.0.0.0/0` である、秘密鍵を貼っている、のいずれかがあると `deploy.ps1` は停止する。これは想定された安全装置なので、止まったら値を直す。
...
次に、**What-Ifを実行する前に**「作成されるリソースの種類・名前・個数」を予想し、証跡の「What-If予想と実測」表に書く。予想の材料は `infra/main.bicep`（リソースグループと命名規則 `<projectName>-<environment>-<regionCode>-001`）、`infra/modules/workload.bicep`、`scripts/verify.ps1` の `$requiredTypes` である。答えを先に見ず、自分の読解で埋めること。この予想がStage 3の「理解して作っている」ことの証拠になる。
...
課金・対象サブスクリプション・削除方法（フェーズ7）を再確認してから実行する。

```powershell
./scripts/deploy.ps1 -ParameterFile infra/parameters/dev.bicepparam -Apply
```

証跡へ記録: 実行日時、終了コード、出力の要点（`resourceGroupName`、`vmName`、`publicIpAddress`。サブスクリプションIDやテナントIDはマスク）。
...
**CT-01**: 必須7種のリソースがすべて `PASS`、かつ「SSH source is restricted」が `PASS`。
...
az monitor metrics alert show --resource-group rg-portfolio-dev-jpe-001 --name <上で表示された名前> --query "{enabled:enabled,scopes:scopes,criteria:criteria}" --output json
```

**MN-01**: 対象がこのVM、しきい値80%、`enabled: true`。
...
### OS-01, OS-02: VM内の確認（SSHセッション内で実行）
...
06「VM内確認コマンド」の残り（`df -h`、`free -m`、`journalctl -u nginx`）も実行して要点を記録しておくと、後の運用・障害対応の練習になる。
...
1. 試験結果表の13行がすべて `PASS` / `FAIL` / `BLOCKED` / `NOT RUN` のいずれかで埋まっている（空欄を残さない）。
